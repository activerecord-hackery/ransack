...
module Ransack
  describe DistinctSortsProcessor do
    ...
    describe '#build_select_value' do
      ...
      it 'calls build_ordering_select_value for Arel ordering node' do
        sort = Person.arel_table[:name].asc
        expect(processor).to receive(:build_ordering_select_value).with(sort, alias_name)
        processor.send(:build_select_value, sort, alias_name)
      end

      it 'calls build_string_select_value for string sort' do
        sort = 'name ASC'
        expect(processor).to receive(:build_string_select_value).with(sort, alias_name)
        processor.send(:build_select_value, sort, alias_name)
      end
    ...
    describe '#build_ordering_select_value' do
      ...
      it 'returns nil for non-Arel attribute' do
        non_attribute_sort = Arel.sql('COUNT(*)').asc
        result = processor.send(:build_ordering_select_value, non_attribute_sort, alias_name)
        expect(result).to be_nil
      end
    end

    describe '#build_string_select_value' do
      let(:alias_name) { 'alias_abc123' }

      it 'removes ORDER BY clauses and adds alias' do
        sort = 'name ASC'
        result = processor.send(:build_string_select_value, sort, alias_name)
      ...
      it 'handles DESC with NULLS' do
        ...
        result = processor.send(:build_string_select_value, sort, alias_name)
        expect(result.to_s).to include('created_at AS alias_abc123')
